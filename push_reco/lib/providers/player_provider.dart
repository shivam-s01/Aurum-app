...
class PlayerProvider extends ChangeNotifier {
  ...
  void _onSongChanged(int index) {
    ...
    // Fire song-started signal so RecommendationEngine updates session state
    // and language/mood lock stays current
  ...
  Future<void> _maybeExtendQueue(int index) async {
    ...
    // Trigger when ≤8 songs remain (was 5) — gives more breathing room
    // Also trigger if queue is brand new (length < 3) — fill immediately
    final shouldExtend = q.length < 3 || remaining <= 8;
    if (!shouldExtend || _isExtendingQueue) return;
    ...
    try {
      ...
      // Full dedup: ALL existing queue IDs + session window
      final existingIds = <String>{
        ...q.map((s) => s.id),
        ...RecommendationEngine.sessionRecentIds,
      };

      // Fetch 50 songs — Spotify-grade queue depth
      ...
      // Final dedup safety check against live queue
      ...
      // Prefetch next 3 songs' stream URLs for gapless feel
      if (toAdd.isNotEmpty) {
        ApiService.prefetchQueue(toAdd.take(3).toList());
        notifyListeners();
      }
    } catch (_) {
