...
class UpdateService {
  ...
  static Future<void> checkForUpdate(BuildContext context) async {
    try {
      // Get current build number from app itself
...
class _UpdateDialogState extends State<_UpdateDialog> {
  ...
  Future<void> _downloadAndInstall() async {
    ...
    try {
      // Use app-specific cache directory — FileProvider can serve this
      ...
      // Verify file is valid (APK must be > 1MB)
      final file = File(path);
      final size = await file.length();
      if (size < 1024 * 1024) {
        setState(() {
          _downloading = false;
          _status = 'Download failed: file invalid. Try again.';
        });
        return;
      }
      ...
      setState(() {
        _downloading = false;
        _status = 'Failed. Check connection and try again.';
      });
    }
  }
