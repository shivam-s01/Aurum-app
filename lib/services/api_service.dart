...
class ApiService {
  ...
  static Future<String?> _saavnStreamById(String songId) async {
    // The Flask backend (cyberboysumanjay/JioSaavnAPI — both onrender and
    // the CF worker) has NO lookup-by-id endpoint; /api/songs?ids= and
    // onrender's own /song/?id= both 404 on it (that shape belongs to a
    // different, Node-style API that isn't what's deployed). Both dead
    // attempts removed — go straight to the CF worker's /song/?id=, which
    // adds its own id-based lookup shim in front of the same Flask API.
    try {
      final res = await _client
