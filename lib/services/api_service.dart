...
class ApiService {
  ...
  /// TEMP DEBUG: raw search test to diagnose empty results
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        if (parsed.isNotEmpty) {
          ...
          final tabs = _path(data, ['contents', 'tabbedSearchResultsRenderer', 'tabs']) as List?;
          buf.writeln('tabs is null: ${tabs == null}, tabs length: ${tabs?.length}');
          buf.writeln('Top-level keys: ${data.keys.join(", ")}');
        }
      } else {
        buf.writeln('Body: ${res.body.substring(0, res.body.length > 500 ? 500 : res.body.length)}');
      }

      buf.writeln('\n--- STREAM RESOLVE TEST ---');
      // dQw4w9WgXcQ is a known-valid YT video id (Rick Astley) just for connectivity test
      final streamUrl = await _ytStreamUrl('dQw4w9WgXcQ');
      buf.writeln('Stream URL resolved: ${streamUrl != null}');
      if (streamUrl != null) {
        buf.writeln('URL starts with: ${streamUrl.substring(0, streamUrl.length > 80 ? 80 : streamUrl.length)}');
      }

      buf.writeln('\n--- FAILURE LOG ---');
      for (final entry in FailureAnalytics.recentLog.take(10)) {
