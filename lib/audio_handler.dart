...
class AurumAudioHandler extends BaseAudioHandler with QueueHandler, SeekHandler {
  // Android-only audio effects. AndroidEqualizer gives the 10-band EQ control
  // the settings screen already exposes; AndroidLoudnessEnhancer backs the
  // Bass Boost toggle (boosts low-end loudness — closer to what users expect
  // from a "Bass Boost" switch than EQ low-band gain alone, and works even
  // when a custom EQ preset doesn't push the bass bands up).
  // Both must be created and passed into AudioPipeline at construction time —
  // just_audio doesn't allow attaching effects after the player exists.
  ...
  Future<void> _applySettings() async {
    ...
    await _applyAudioEffects(p);
  }

  // Pushes the 10-band EQ (eq_band_0..9, dB, -12..+12) and Bass Boost toggle
  // saved by the settings screen into the actual Android audio effects
  // chain. Previously these prefs were written but never read anywhere —
  // the sliders moved in the UI but had zero effect on playback.
  Future<void> _applyAudioEffects(SharedPreferences p) async {
    try {
      await _equalizer.setEnabled(true);
      final params = await _equalizer.parameters;
      final bandCount = params.bands.length;
      // Map our fixed 10 saved bands onto however many bands this device's
      // equalizer actually exposes (most Android EQs report 5; some 6).
      for (int i = 0; i < bandCount; i++) {
        final savedIndex = bandCount == 10
            ? i
            : (i * 10 / bandCount).floor().clamp(0, 9);
        final gainDb = p.getDouble('eq_band_$savedIndex') ?? 0.0;
      ...
      // AndroidEqualizer is Android-only and can fail to initialize on
      // some OEM skins/emulators — never let EQ failure break playback.
      debugPrint('[AurumHandler] Equalizer apply failed (ignored): $e');
    }
    ...
    try {
      ...
      if (bassBoost) {
        // setTargetGain takes decibels. ~9dB is a strong, clearly audible
        // boost without obvious clipping/distortion on most devices.
        await _loudnessEnhancer.setTargetGain(9.0);
      }
    } catch (e) {
      debugPrint('[AurumHandler] LoudnessEnhancer apply failed (ignored): $e');
    }
  }
  ...
  Future<void> playQueue(List<Song> songs, int startIndex) async {
    ...
    try {
      ...
      try {
        ...
        debugPrint('[AurumHandler] setAudioSource threw for "${songs[safeIndex].title}": '
            '${_exceptionDetail(e)} — retrying with forced-refresh URL');
        if (mySession != _playSessionId) return;

        // setAudioSource can throw immediately (PlayerException code=0
        // "Source error") even after a URL passed our liveness check —
        // the HEAD/range probe and the real GET can land on different CDN
        // edges, or the URL can die in the seconds between check and use.
        // One forced-refresh retry catches most of these transient cases
        // instead of failing the song outright.
        try {
          final retrySong = songs[safeIndex];
          final retryUrl = await ApiService.resolveStreamUrl(retrySong, forceRefresh: true)
              .timeout(const Duration(seconds: 15), onTimeout: () => null);
          if (retryUrl == null || mySession != _playSessionId) {
            _failPlayback(retrySong, _exceptionDetail(e), prefix: 'setAudioSource failed');
            return;
          }
          final retrySource = AudioSource.uri(Uri.parse(retryUrl), tag: _songToMediaItem(retrySong));
          final retryFresh = ConcatenatingAudioSource(children: [retrySource]);
          await _player.setAudioSource(retryFresh, initialIndex: 0, preload: false);
          debugPrint('[AurumHandler] setAudioSource retry succeeded for "${retrySong.title}" ✓');
        } catch (e2) {
          _failPlayback(songs[safeIndex], _exceptionDetail(e2), prefix: 'setAudioSource failed (after retry)');
          return;
        }
      }
      if (mySession != _playSessionId) return;
  ...
  Future<void> playSong(Song song) async {
    ...
    try {
      ...
      try {
        ...
        debugPrint('[AurumHandler] setAudioSource threw for "${song.title}": '
            '${_exceptionDetail(e)} — retrying with forced-refresh URL');
        if (mySession != _playSessionId) return;

        try {
          final retryUrl = await ApiService.resolveStreamUrl(song, forceRefresh: true)
              .timeout(const Duration(seconds: 15), onTimeout: () => null);
          if (retryUrl == null || mySession != _playSessionId) {
            _failPlayback(song, _exceptionDetail(e), prefix: 'setAudioSource failed');
            return;
          }
          final retrySource = AudioSource.uri(Uri.parse(retryUrl), tag: _songToMediaItem(song));
          final retryFresh = ConcatenatingAudioSource(children: [retrySource]);
          await _player.setAudioSource(retryFresh, initialIndex: 0, preload: false);
          debugPrint('[AurumHandler] setAudioSource retry succeeded for "${song.title}" ✓');
        } catch (e2) {
          _failPlayback(song, _exceptionDetail(e2), prefix: 'setAudioSource failed (after retry)');
          return;
        }
      }
      if (mySession != _playSessionId) return;
