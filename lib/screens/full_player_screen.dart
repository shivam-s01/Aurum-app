...
class _FullPlayerScreenState extends State<FullPlayerScreen>
  ...
  void _applyPalette(ArtworkPalette p, {required int gen, required bool isLight, bool instant = false}) {
    ...
    if (isLight) {
      // Echo-Nightly-exact: light mode reads from the palette's OWN
      // light swatch (lightVibrant, already a soft/pastel tone as
      // generated by the palette extractor) rather than lifting the
      // full-saturation vibrant color by a small amount — that's what
      // was making light mode read as "glowing"/oversaturated compared
      // to Echo, which never puts a strong color in front of light-mode
      // white. c4 (lightVibrant) is now the base for bg1 (the color the
      // vignette/lift painter actually uses), with a much higher white
      // lift so it always stays a gentle, controlled tint no matter how
      // saturated the source artwork is.
      _targetBg1 = ensureContrastSafe(Color.lerp(c4, Colors.white, 0.55)!, isLight: true);
      _targetBg2 = ensureContrastSafe(Color.lerp(c2, Colors.white, 0.45)!, isLight: true);
      _targetBg3 = ensureContrastSafe(Color.lerp(c3, Colors.white, 0.30)!, isLight: true);
      _targetBg4 = ensureContrastSafe(Color.lerp(c4, Colors.white, 0.55)!, isLight: true);
    } else {
...
class _LightVignettePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // artwork rather than a generic overlay. tint is now already a
    // heavily white-lifted lightVibrant swatch (see _applyPalette), so
    // this only needs a light extra touch, not a second full lift —
    // stacking two strong lifts on top of each other was the main
    // source of the "too much glow" look compared to Echo Nightly's
    // single flat tint.
    final liftColor = Color.lerp(Colors.white, tint, 0.10)!.withAlpha(16);
    canvas.drawRect(fullRect, Paint()..color = liftColor);

    final vignetteTop = Color.lerp(Colors.white, tint, 0.08)!.withAlpha(46);
    final vignetteBottom = Color.lerp(Colors.white, tint, 0.08)!.withAlpha(60);
    final vignettePaint = Paint()
      ..shader = LinearGradient(
        begin: Alignment.topCenter,
        end: Alignment.bottomCenter,
